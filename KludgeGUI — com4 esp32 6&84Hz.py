...
def generate_wave_points(waveform,frequency,amplitude,offset):
    ...
    samples_per_cycle_six_hz = int(round(1/(float(frequency_six_hz) * .000512)))
    wave_list = [0]* samples_per_cycle_six_hz
    if(waveform == "six_hz_and_sine"):
        for x in range (0, samples_per_cycle_six_hz):
            wave_list[x] = amplitude*(math.sin(2*(math.pi)*(x/samples_per_cycle_six_hz))) + offset
#            if((x >= samples_per_cycle_six_hz/8) and (x < samples_per_cycle_six_hz/2 - samples_per_cycle_six_hz/8)):
            if((x >= samples_per_cycle_six_hz/8 + samples_per_cycle_six_hz/2) and (x < samples_per_cycle_six_hz/2 + samples_per_cycle_six_hz/2 - samples_per_cycle_six_hz/8)):
                wave_list[x] = wave_list[x] + (amplitude*3/4)*(math.sin(2*(math.pi)*(x/samples_per_cycle))) + offset
        wave_list[samples_per_cycle_six_hz - 1] = (wave_list[0] + wave_list[samples_per_cycle_six_hz - 2])/2.0
    if(waveform == "sine"):
...
def updateGUI():
    ...
    introtext = font.render("[     6Hz & SINE WAVE    ]" + addindicator("six_hz_and_sine"), 0,setactivecolor("six_hz_and_sine"))
    sixhzandsinebutton = displaysurface.blit(introtext, (5, 480))    
    ...
    if(which_mode != "random"):
        for x in range (0,300):
            pygame.draw.line(displaysurface,(240,240,255),(10+(5*(x/3)),100 - (40*wave_points[round((x/3)*(listlength/40)) % listlength])),(15 +(5*(x/3)),100 - (40*wave_points[round(((x/3)+1)*(listlength/40)) % listlength])),2)
    else:
        for x in range (0,100):
            pygame.draw.line(displaysurface,(240,240,255),(10+(5*x),100 - (40*wave_points[round(x*(listlength/40)) % listlength])),(15 +(5*x),100 - (40*wave_points[round(x*(listlength/40)) % listlength])),2)

    if(which_mode == "random"):
        introtext = font.render("1 value per 1024 us", 0, (120,120,120))
    else:
        if(which_mode == "dc"):
            introtext = font.render(" ",0,(0,0,0))
        else:
            if(frequency < 1):
                introtext = font.render(str(round(1/frequency,2)) + " seconds",0,(120,120,120))
            else:
                introtext = font.render(str(round(1000/frequency,2)) + " milliseconds",0,(120,120,120))
    displaysurface.blit(introtext,(305,143))
    ...
    for event in pygame.event.get():
        ...
        if(event.type == pygame.MOUSEBUTTONDOWN and event.button == 1):
            ...
            if(randombutton.collidepoint(cursor)):
                which_mode = "random"
                updatewave = 1
            if(sixhzandsinebutton.collidepoint(cursor)):
                wave_points = generate_wave_points("six_hz_and_sine",frequency,2,0)
                which_mode = "six_hz_and_sine"
                updatewave = 1
...
def initializeGUI():
    pygame.init()
    global displaysurface
    displaysurface = pygame.display.set_mode((640, 510))
...
global wave_points
wave_points = generate_wave_points("six_hz_and_sine",84,0.5,0)
